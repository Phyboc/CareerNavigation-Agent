...
export async function callLLM(systemPrompt, userPrompt, options = {}) {
	...
	// One retry for transient upstream failures (rate limits / 5xx).
...
- projects: array of project titles or short descriptions from the resume
...
- matchScore: number from 0 to 100`;
	...
	if (result && typeof result === "object") {
		...
		// Drop entries that look like project descriptions or sentences rather
		// than skills (e.g. "Drowsiness Detection System development").
		if (Array.isArray(result.detectedSkills)) {
			result.detectedSkills = result.detectedSkills.filter(isPlausibleSkill);
		}
