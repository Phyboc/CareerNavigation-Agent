...
import { generateResumeAnalysis } from "./aiProvider";
...
export async function analyzeCareerProfile(profile = {}) {
  ...
  // If resume text/data is provided, integrate detected resume skills/projects using AI.
  if (profile.resumeText) {
    try {
      const resumeAnalysis = await generateResumeAnalysis(profile.resumeText, normalizedProfile.goal);
      if (resumeAnalysis && typeof resumeAnalysis === "object") {
        ...
        if (Array.isArray(resumeAnalysis.projects) && resumeAnalysis.projects.length) {
          const mergedProjects = [...new Set([...(normalizedProfile.projects || []), ...resumeAnalysis.projects])];
          normalizedProfile.projects = mergedProjects;
        }
        normalizedProfile._resumeAnalysis = resumeAnalysis;
      }
    } catch (e) {
      // Fallback to original static analysis if AI call fails.
      try {
        const fallback = analyzeResumeText(profile.resumeText, normalizedProfile.goal);
