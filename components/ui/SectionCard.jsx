export default function SectionCard({ eyebrow, title, description, children, className = "" }) {
	return (
		...
		>
			{eyebrow ? (
				<p className="text-sm font-medium tracking-wide text-cyan-700">{eyebrow}</p>
			) : null}
			{title ? <h3 className="mt-2 text-2xl font-bold tracking-tight text-slate-100 font-display sm:text-3xl">{title}</h3> : null}
			{description ? (
