	...
	return (
		<section className="glass-panel rounded-[32px] p-6 sm:p-8 md:p-10 transition duration-300 hover:border-cyan-600/25">
			<div className="text-center">
				...
				<h2 className="mt-3 font-display text-3xl font-bold text-slate-100 sm:text-4xl">Progress from student to industry ready</h2>
			...
			<div className="mt-10 flex flex-col items-center gap-4 md:flex-row md:justify-between relative">
				...
				<div className="absolute top-6 left-8 right-8 h-0.5 bg-slate-700/20 z-0 hidden md:block" />
					...
					<div key={stage} className="flex flex-col items-center md:flex-1 relative z-10 w-full md:w-auto">
						<div className="flex flex-col items-center">
							<div className={`flex h-12 w-12 items-center justify-center rounded-xl border text-sm font-bold transition duration-300 ${
								index <= 2 
									? "border-cyan-600/35 bg-cyan-600/10 text-cyan-800 shadow-[0_0_15px_rgba(15,118,110,0.15)]" 
									: "border-slate-700/25 bg-slate-900/60 text-slate-500"
							}`}>
								{index + 1}
							</div>
							<p className="mt-3 font-display text-xs font-bold tracking-wide text-slate-300">{stage}</p>
						</div>
