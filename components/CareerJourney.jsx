	...
	return (
		...
		>
			<div className="mt-8 flex flex-col md:flex-row md:items-start md:justify-between relative gap-6 md:gap-0">
				...
				<div className="absolute top-7 left-12 right-12 h-[2px] bg-slate-700/20 z-0 hidden md:block" />
						...
						<div key={stage.label} className="flex flex-col items-center md:flex-1 relative z-10 w-full md:w-auto">
							<div className="flex flex-col items-center">
								<div
									className={`flex h-14 w-14 items-center justify-center rounded-2xl border-2 text-base font-bold transition-all duration-300 ${
										current
											? "border-cyan-600 bg-cyan-600/15 text-cyan-900 shadow-[0_0_24px_rgba(15,118,110,0.25)]"
											: active
												? "border-cyan-600/35 bg-cyan-600/10 text-cyan-800"
												: "border-slate-700/25 bg-slate-900/60 text-slate-500"
									}`}
								>
									{index + 1}
								</div>
								<p className={`mt-4 text-center text-sm font-semibold font-display tracking-wide ${active ? "text-slate-100" : "text-slate-500"}`}>
									{stage.label}
								</p>
								{current ? (
									<span className="mt-1.5 rounded-full border border-cyan-600/25 bg-cyan-600/10 px-3 py-0.5 text-[10px] uppercase font-bold tracking-wider text-cyan-800">
										You are here
									</span>
								) : null}
							</div>
							{index < STAGES.length - 1 ? (
								<div className="my-2 flex h-8 items-center md:hidden">
									<div className={`h-full w-0.5 ${active ? "bg-cyan-600/40" : "bg-slate-700/20"}`} />
								</div>
							) : null}
						</div>
					);
			...
			<p className="mt-8 text-center text-sm text-slate-400">
				Current readiness: <span className="font-semibold text-cyan-700">{score}%</span> — keep building to advance to the next stage.
			</p>
		</SectionCard>
	);
}
