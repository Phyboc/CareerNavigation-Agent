export default function Roadmap({ roadmap = [] }) {
  return (
    <section className="rounded-[32px] border border-white/10 bg-slate-950/80 p-6 shadow-[0_20px_60px_rgba(2,6,23,0.22)] backdrop-blur">
      <p className="text-xs font-semibold uppercase tracking-[0.24em] text-cyan-300">Learning roadmap</p>
      <h3 className="mt-2 text-2xl font-semibold text-white">Four-phase plan</h3>
      <div className="mt-6 grid gap-4 xl:grid-cols-4">
        {roadmap.map((phase, index) => (
          <article key={`${phase.phase}-${index}`} className="rounded-3xl border border-white/10 bg-white/5 p-5">
