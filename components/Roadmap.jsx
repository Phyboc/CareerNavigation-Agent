export default function Roadmap({ roadmap = [] }) {
  return (
    <section className="rounded-[32px] border border-white/10 bg-slate-950/80 p-6 shadow-[0_20px_60px_rgba(8,145,178,0.07)] backdrop-blur transition duration-200 hover:border-cyan-400/20 hover:shadow-[0_24px_60px_rgba(8,145,178,0.12)]">
      <p className="text-sm font-medium tracking-wide text-cyan-200">Learning roadmap</p>
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-4">
        {roadmap.length > 0 ? roadmap.map((phase, index) => (							<article key={`${phase.phase}-${index}`} className="rounded-3xl border border-white/10 bg-white/5 p-5 transition duration-200 hover:border-cyan-400/20 hover:bg-white/7 flex flex-col h-full justify-between">
            <div className="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-cyan-400 to-cyan-600 text-sm font-bold text-slate-950">0{index + 1}</div>
